<template>
  <div class="flex flex-col py-5 px-2 bg-gray-500 to-do-list-side-bar">
    <div v-for="({ id, name }, i) in toDoLists" :key="i" class="flex">
      <RouterLink :key="id" class="w-1/2 capitalize" :to="`/to-do-list/${id}`">
        <li> {{ name }} </li>
      </RouterLink>
    ...
    <div class="mt-5">
      <input v-model="newList" class="border-2 border-green-400 border-radius focus:" placeholder="add new list" type="text" @keyup.enter="store.addList">
